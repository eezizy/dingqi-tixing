...
export function getSettings(){
  ...
  return {
    ...
    auth_user:     s.auth_user     || process.env.AUTH_USER     || ''
  };
}
...
export function getAuth(){
  const s = readJson(SETTINGS_FILE, {});
  return {
    user: s.auth_user || process.env.AUTH_USER || '',
    pass: s.auth_pass || process.env.AUTH_PASS || ''
  };
}
export function saveAuth(user, pass){
  const s = readJson(SETTINGS_FILE, {});
  const cur = getAuth();
  const u = user || cur.user || '';     // 不允许清空账号（避免误开放）
  s.auth_user = u;
  if(pass) s.auth_pass = pass;          // 密码留空则不覆盖原密码
